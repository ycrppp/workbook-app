...
Диагностика: интро уже назвало какой разрыв объясняет боль пользователя. Задача ex1 — не открывать заново, а углублять: пользователь описывает как именно этот разрыв проявляется в его конкретной работе — 2-3 примера из реальной практики последних недель. Никаких списков "выбери один из трёх" — только его ситуация.
Инструмент: пользователь называет конкретную неправильную реакцию которую применяет к этому разрыву — и разбирает какое трение она создаёт именно в его работе. Результат — чёткая формулировка: реакция + трение которое она создаёт. Не анализ в общем — конкретный личный диагноз.
Следующий шаг: в одной из ситуаций которые разобрал, применяет правильную реакцию по Бангею вместо той которую описал в предыдущем упражнении. Конкретная ситуация + конкретный день. Если действие связано с другим человеком — называет кого. Если это личное поведенческое изменение — просто ситуация и день без обязательного человека. Формат: что сделает + когда + (кому, если есть).`,
      },
      {
        id: 'intent',
...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь смотрит на свою конкретную боль и находит где сам сейчас работает без чёткого замысла — ставит задачи без зачем (если есть кому ставить), планирует без намерения, или общается с партнёрами/клиентами без замысла. 2-3 конкретных примера из реальной практики.
Инструмент: берёт ту ситуацию из своей боли которая болит сильнее всего и записывает для неё замысел по структуре Бангея — это реальный текст который он может использовать прямо сейчас.
Следующий шаг: берёт замысел который написал выше и применяет его в одной конкретной ситуации на этой неделе — в момент когда обычно работал бы без замысла. Конкретная ситуация + конкретный день. Формат: в какой момент + что именно сделает по-другому.`,
      },
      {
        id: 'cascade',
        title: 'Каскад целей',
...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь смотрит на свою боль и находит конкретные случаи: если ставит задачи — где говорит КАК вместо ЗАЧЕМ; если сам исполнитель — где получает КАК без ЗАЧЕМ и как это мешает адаптироваться. Минимум 3 примера из реальной практики.
Инструмент: берёт один из этих случаев — самый болезненный — и переформулирует задачу в замысел: только ЧТО и ЗАЧЕМ, без КАК. Для исполнителя — переформулирует полученную задачу так чтобы понять зачем она нужна. Это готовый текст который он может использовать.
Следующий шаг: если ставит задачи — одно конкретное решение которое передаст с замыслом вместо инструкции на этой неделе: кому, что, в каком формате. Если сам исполнитель — один конкретный разговор где применит briefing back или попросит уточнить замысел: с кем, по какой задаче, когда.`,
      },
      {
        id: 'independence',
...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь вспоминает 3 конкретных недавних случая из своей боли: если руководит — когда человек пришёл за решением вместо того чтобы действовать самостоятельно; если сам исполнитель или соло — когда сам шёл за согласованием вместо того чтобы действовать. Для каждого: был ли понятен замысел? Что помешало действовать самостоятельно — отсутствие замысла или страх последствий?
Инструмент: берёт один из этих случаев и пишет что именно изменит: если руководит — как конкретно покажет что инициатива в рамках замысла безопасна (не декларацией, а конкретным действием или словами); если сам исполнитель — что именно решит самостоятельно в следующий раз и как проверит что это в рамках замысла. Конкретный текст или договорённость.
Следующий шаг: один конкретный момент на этой неделе — ситуация, день, что именно сделает по-другому. Если это разговор с конкретным человеком — называет с кем и о чём. Если это личное решение которое примет без согласования — называет ситуацию без обязательного человека.`,
      },
    ],
  },
};
