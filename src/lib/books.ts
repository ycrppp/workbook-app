...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь смотрит на свою конкретную боль и находит где сам сейчас ставит задачи без зачем — 2-3 конкретных примера из реальной практики с командой, подрядчиками или клиентами.
...
Следующий шаг: берёт замысел который написал выше и применяет его в одной конкретной ситуации на этой неделе — вместо того чтобы просто поставить задачу. Конкретная ситуация + конкретный день. Формат: в какой момент + что именно сделает по-другому.`,
      },
      {
        id: 'cascade',
        title: 'Каскад целей',
...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь смотрит на свою боль и находит конкретные случаи где говорит КАК вместо ЗАЧЕМ — минимум 3 примера из реальной практики с командой, подрядчиками или партнёрами.
Инструмент: берёт один из этих случаев — самый болезненный — и переформулирует задачу в замысел: только ЧТО и ЗАЧЕМ, без КАК. Это готовый текст который он может использовать.
Следующий шаг: одно конкретное решение которое передаст на этой неделе с замыслом вместо инструкции — кому конкретно, что, в каком формате.`,
      },
      {
        id: 'independence',
...
ПРИМЕНЕНИЕ К ТРЁМ УПРАЖНЕНИЯМ (все три про боль пользователя):
Диагностика: пользователь вспоминает 3 конкретных недавних случая — когда человек пришёл за решением вместо того чтобы действовать самостоятельно. Для каждого: был ли понятен замысел? Что помешало действовать самостоятельно — отсутствие замысла или страх последствий?
Инструмент: берёт один из этих случаев и пишет что конкретно изменит: как покажет что инициатива в рамках замысла безопасна — не декларацией, а конкретным действием или словами. Конкретный текст или договорённость.
Следующий шаг: один конкретный момент на этой неделе — ситуация, день, что именно сделает по-другому. Если это разговор — называет с кем. Если это личное решение — называет ситуацию.`,
      },
    ],
  },
};
