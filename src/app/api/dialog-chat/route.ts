...
const EX_GOAL: Record<string, string> = {
  ex1: 'Упражнение — ДИАГНОСТИКА. Каждый твой ответ состоит из двух частей: (1) наблюдение — что в его ответе уже есть и что осталось размытым или не названным относительно задания, (2) один вопрос — который помогает заполнить главный пробел. Наблюдение строго по тексту его ответа и заданию — не додумывай. Цель — чтобы ответ точнее описывал его реальную ситуацию через концепцию модуля.',
  ex2: 'Упражнение — ИНСТРУМЕНТ. Каждый твой ответ состоит из двух частей: (1) наблюдение — какой элемент инструмента заполнен хорошо, а какой остался размытым или пропущенным, (2) один вопрос — про самый слабый элемент. Наблюдение строго по тексту его ответа и заданию. Цель — чтобы артефакт упражнения получился рабочим и применимым прямо сейчас.',
